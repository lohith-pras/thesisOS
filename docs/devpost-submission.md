...
Proofline also keeps the surrounding work locations intentional: researchers can choose or create a VS Code workspace, choose or scaffold an Obsidian research vault, and store an Overleaf project URL. Each connection is optional and project-scoped. VS Code opens the configured local folder; Obsidian receives previewed, explicitly approved notes; Overleaf opens in the browser without credentials, syncing, or remote edits from Proofline.

## How we built it

The project is a Node.js local-first application. Codex CLI turns messy feedback into a validated task graph and drafts grounded notes without requiring API credits. Zotero Desktop is accessed through its read-only local API, including personal and group-library discovery. Local Ollama embeddings provide semantic retrieval, with content-addressed caching and a visible metadata fallback. Codex uses strict structured output to draft only from selected evidence; unknown source IDs invalidate the draft. GPT-5.6 remains an optional API adapter. Finder-backed folder selection configures VS Code and Obsidian; new Obsidian vaults receive a research scaffold. Overleaf is deliberately URL-only, so the app never receives an Overleaf credential.
